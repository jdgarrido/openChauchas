;(function($, undefined){
    ...
    Tagit.prototype = {
        ...
        addTagInput : function(){
            this.tagInput = $('<input />')
                ...
                .addClass(this.options.newTagInputClass)
                .val('');
    ...
    $(function(){
        var tagitInstances = $('ul[data-toggle="tagit"]');
        if(tagitInstances.length){
            tagitInstances.each(function(){
                var tagitInstance = $(this),
                    source = tagitInstance.data('tagit-source'),
                    placeholder = tagitInstance.data('tagit-placeholder'),
                    inputClass = tagitInstance.data('tagit-class');

                if(source.slice(0,1) === '$')
                    source = eval(source.slice(1));

                tagitInstance.tagit({
                    tagSource : source,
                    newTagInputText : placeholder,
                    newTagInputClass : inputClass
                });
            });
        }
    });
